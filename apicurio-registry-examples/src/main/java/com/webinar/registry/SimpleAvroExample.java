...
public class SimpleAvroExample {
    ...
    private static int numOfMessages = 5;



    private static String registryURL = "http://localhost:8080/apis/registry/v2";
    private static String brokers = "localhost:9092";
    private static String topicName = SimpleAvroExample.class.getSimpleName();
    private static String subjectName = "Greeting";
    private static String schemaValue = "{\"type\":\"record\",\"name\":\"Greeting\",\"fields\":[{\"name\":\"Message\",\"type\":\"string\"},{\"name\":\"Time\",\"type\":\"long\"}]}";
    ...
    private static String AssignEnv(String env, String def) {
        String envVal = System.getenv(env);
        return envVal == null ? def : envVal;
    }

    private static void GetEnv() {
        numOfMessages = Integer.parseInt(AssignEnv("NUMBER_OF_MESSAGES", String.valueOf(numOfMessages)));

        brokers = AssignEnv("KAFKA_BROKERS", brokers);
        topicName = AssignEnv("KAFKA_TOPIC", topicName);
        subjectName = AssignEnv("KAFKA_SUBJECT", subjectName);
        schemaValue = AssignEnv("KAFKA_SCHEMA", schemaValue);
        registryURL = AssignEnv("KAFKA_REGISTRY_URL", registryURL);

        //Kafka Security
        username = AssignEnv("KAFKA_USERNAME", username);
        password = AssignEnv("KAFKA_PASSWORD", password);
        securityProtocol = AssignEnv("KAFKA_SECURITY_PROTOCOL", securityProtocol);
        saslMechanism = AssignEnv("KAFKA_SASL_MECHANISM", saslMechanism);
    }
    ...
    public static final void main(String[] args) throws Exception {
        ...
        try {
            ...
            System.out.printf("Producing (%d) messages.\n", numOfMessages);
            for (int idx = 0; idx < numOfMessages; idx++) {
        ...
        try {
            int messageCount = 0;
            System.out.printf("Consuming (%d) messages.", numOfMessages);
    ...
    private static Producer<Object, Object> createKafkaProducer() {
        ...
        props.putIfAbsent(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, brokers);
    ...
    private static KafkaConsumer<Long, GenericRecord> createKafkaConsumer() {
        ...
        // Configure Kafka
        props.putIfAbsent(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, brokers);
    ...
    private static void configureSecurityIfPresent(Properties props) {
        props.putIfAbsent("security.protocol", securityProtocol);
        if(securityProtocol.contains("SASL")) {
            props.putIfAbsent(SaslConfigs.SASL_MECHANISM, saslMechanism);

            if(saslMechanism.toLowerCase().contains("plain")) {
            ...
            } else if(saslMechanism.toLowerCase().contains("scram-sha-256")) {
                props.putIfAbsent(SaslConfigs.SASL_JAAS_CONFIG, String.format("org.apache.kafka.common.security.scram.ScramLoginModule required username=\"%s\" password=\"%s\";", username, password));
            } else {
                System.out.println("Mechanism not supported: " + saslMechanism);
                System.exit(0);
            }
        }
